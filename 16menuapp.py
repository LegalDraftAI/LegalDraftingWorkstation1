...
with tab_work:
    ...
    with st.sidebar:
        ...
        st.divider()
        uploaded = st.file_uploader("Vault Ref Doc", type="docx")
        if uploaded:
    ...
    with c1:
        if st.button("🔍 Search API", use_container_width=True):
            st.session_state.search_results = [{"title": "Kerala HC Precedent 2025", "cite": "2025 KER 101", "sum": "Relevant citation."}]
    with c2:
    ...
    if st.session_state.final_master:
        st.divider()
        st.subheader("🛠️ Name Replacement")
        m1, m2 = st.columns(2)
        with m1:
            name_a = st.text_input("Name for PARTY A (Petitioner):")
            if st.button("Update Party A"):
        ...
        with e2:
            st.download_button("📥 MS Word (.docx)", data=create_docx(st.session_state.final_master), file_name="draft.docx", use_container_width=True)
        with e3:
            st.download_button("📥 Adobe PDF (.pdf)", data=create_pdf(st.session_state.final_master), file_name="draft.pdf", use_container_width=True)