...
def reset_workstation():
    ...
    st.session_state.widget_key += 1 # This forces the Facts text area to empty
...
with tab_work:
    with st.sidebar:
        ...
        # High Court Specific District Lock Logic
    ...
    # Success Celebration Signal
    if st.session_state.draft_done:
        st.balloons()
        st.success("✅ Drafting Complete! Latest template is ready below.", icon="⚖️")
        st.session_state.draft_done = False

    st.title(f"Drafting: {doc_type}")
    
    # Facts area with key-based deep reset
    facts = st.text_area(
        "Case Facts:", 
        placeholder="Briefly describe the matter or paste citations...", 
        height=150, 
        key=f"facts_input_{st.session_state.widget_key}"
    )

    # 5. ACTION ROW (Search, Research, Draft)
    c1, c2, c3 = st.columns([1, 1, 1])
    
    with c1:
        # Automated API Search (Latest First)
        if st.button("🔍 Search API", use_container_width=True):
            with st.spinner("Searching Precedents..."):
                if len(st.session_state.ik_token) < 5:
                    st.session_state.search_results = [{"title": "Intern Mode: API Token Missing", "headline": "Enter your API token in Settings to see automated, live results here."}]
                else:
                    try:
                        headers = {'Authorization': f'Token {st.session_state.ik_token}'}
                        # Query forced to most recent
                        r = requests.post(f"https://api.indiankanoon.org/search/?formInput={facts}+sortby:mostrecent", headers=headers)
                        if r.status_code == 200: 
                            st.session_state.search_results = r.json().get('docs', [])
                    except: st.error("Search API Connection failed.")
            st.rerun()

    with c2:
        # Web Research (Existing Browser Login - Latest First)
        if facts:
            encoded_query = urllib.parse.quote_plus(facts)
            search_url = f"https://indiankanoon.org/search/?formInput={encoded_query}&sortby=mostrecent"
            st.link_button("🌐 Research & Login", search_url, use_container_width=True, help="Opens Indian Kanoon website. If not logged in, it will prompt for credentials then show latest cases first.")
        else:
            st.button("🌐 Research & Login", disabled=True, use_container_width=True)

    with c3:
        # AI Drafting with Generation Timer
        if st.button("🚀 Draft Petition", type="primary", use_container_width=True):
            if not st.session_state.google_key:
                st.error("Please add your Gemini Key in Settings tab.")
            else:
                with st.spinner("Senior Advocate is drafting... please wait.", show_time=True):
                    try:
                        ...
                        2. NEVER invent real names, addresses, or dates.
                        3. Facts to base drafting on: {facts}
                        """
    ...
    if st.session_state.search_results:
        with st.expander("📚 Case Precedents (API Results)", expanded=True):
    ...
    if st.session_state.final_master:
        ...
        st.info("💡 Edit the template below to finalize. PARTY A and PARTY B are placeholders.")
        
        edited_text = st.text_area("Final Draft:", value=st.session_state.final_master, height=500, key="editor_window")
