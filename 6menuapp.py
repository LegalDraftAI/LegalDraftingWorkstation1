...
def load_history():
    if os.path.exists(HISTORY_FILE):
        try:
            ...
            # Take last 10 and reverse so newest is at the top
...
with tab_work:
    ...
    if st.session_state.draft_done:
        st.balloons()
        st.success("✅ Drafting Complete! Template is ready in the Editor.", icon="⚖️")
        st.session_state.draft_done = False

    st.title(f"Drafting: {doc_type}")
    facts = st.text_area("Case Facts:", placeholder="Briefly describe the matter...", height=150)
        ...
        if st.button("🔍 Search Precedents", use_container_width=True):
            with st.spinner("Searching Indian Kanoon..."):
                if len(st.session_state.ik_token) < 5:
                    st.session_state.search_results = [{"title": f"Recent {doc_type} Precedent", "headline": "Mock precedent for demo purposes."}]
                else:
                    try:
            ...
            if not st.session_state.google_key:
                ...
                with st.spinner("Senior Advocate is drafting... please wait.", show_time=True):
                    try:
                        ...
                        RULES:
                        1. Use "PARTY A" for Petitioner and "PARTY B" for Respondent.
                        2. DO NOT invent real names or addresses.
                        3. Use placeholders like [BANK NAME] or [DATE] for specific details.
                        FACTS: {facts}
                        """
    ...
    # PRECEDENTS DISPLAY (Remains visible while drafting)
    ...
    if st.session_state.final_master:
        ...
        st.info("💡 Edit the placeholders below before downloading.")
        ...
        with ec1:
            if st.button("💾 Save to Local History"):
                ...
                st.rerun() # Refresh sidebar history immediately
        with ec2:
